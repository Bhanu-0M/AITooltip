...
var hoverZoom = {
    ...
    loadHoverZoom:function () {
        ...
        function loadFullSizeImage() {
            ...
            if (!imgFullSize) {
                ...
                if (!srcDetails.url.startsWith('data:')) {
                    ...
                    srcDetails.host = 'text-summary'; // Placeholder for generated images
                }
        ...
        function displayFullSizeImage() {
            console.log("DIsplay full size image")
            ...
            chrome.runtime.sendMessage({action:'getPermissionsContains', permissions: ['history']},
                function(hasPermission) {
                    // Only add to history if it's not a generated data URL
                    if (hasPermission && !chrome.extension.inIncognitoContext && !srcDetails.url.startsWith('data:')) {
        ...
        function getSrcDetails(link) {
            let details = {};
            if (srcDetails.url && srcDetails.url.startsWith('data:image/')) {
                // Details for generated summary images
                if (srcDetails.naturalWidth && srcDetails.naturalHeight) { // These are set from the canvas dimensions
                    details.dimensions = `${Math.round(srcDetails.naturalWidth)}x${Math.round(srcDetails.naturalHeight)}`;
                    details.ratio = getImgRatio(srcDetails.naturalWidth, srcDetails.naturalHeight);
                    if (imgFullSize && (imgFullSize.width() || (imgFullSize[0] && imgFullSize[0].width))) {
                        let displayedWidth = imgFullSize.width() || imgFullSize[0].width;
                        details.scale = Math.round(100.0 * displayedWidth / srcDetails.naturalWidth) + '%';
                    }
                }
                details.extension = 'PNG'; // We generate PNGs
                details.host = 'Generated Summary';
                details.filename = 'summary.png';
                // No content length or last modified for data URLs
            } else {
                // Existing logic for non-data URLs
                if (!srcDetails.audio || srcDetails.video) {
                    ...
                    if (imgFullSize && (imgFullSize.width() || (imgFullSize[0] && imgFullSize[0].width)) && srcDetails.naturalWidth) {
                        let displayedWidth = imgFullSize.width() || imgFullSize[0].width;
                        details.scale = Math.round(100.0 * displayedWidth / srcDetails.naturalWidth) + '%';
                    }
                }
    ...
    getFullUrl:function (url) {
        url = url.trim();
        if (url.indexOf('http:') !== 0 && url.indexOf('https:') !== 0 && url.indexOf('file:') !== 0 && url.indexOf('blob:http:') !== 0 && url.indexOf('blob:https:') !== 0 && url.indexOf('data:') !== 0) {
            if (url.indexOf('//') !== 0) {
                if (url.indexOf('/') === 0) {
...
// Add a util namespace if it doesn't exist
hoverZoom.util = hoverZoom.util || {};

/**
 * Converts a string of text into a data URL for an image.
 * @param {string} text - The text to render.
 * @param {object} options - Configuration for rendering.
 * @param {string} [options.font='16px Arial'] - CSS font string.
 * @param {number} [options.fontSize=16] - Numeric font size in pixels.
 * @param {string} [options.fontColor='black'] - Color of the text.
 * @param {string} [options.backgroundColor='white'] - Background color of the image.
 * @param {number} [options.maxWidth=600] - Maximum width of the image. Text will wrap.
 * @param {number} [options.lineHeight] - Line height. Defaults to 1.2 * fontSize.
 * @param {number} [options.padding=20] - Padding around the text.
 * @param {number} [options.maxLines=30] - Maximum lines of text to render before truncating.
 * @returns {string} - A data URL (e.g., 'data:image/png;base64,...') for the generated image.
 */
hoverZoom.util.textToImageDataUrl = function(text, options = {}) {
    const defaults = {
        font: options.fontSize ? `${options.fontSize}px Arial` : '16px Arial',
        fontSize: parseInt(options.fontSize || 16),
        fontColor: 'black',
        backgroundColor: 'white',
        maxWidth: 600,
        lineHeight: options.fontSize ? parseInt(options.fontSize || 16) * 1.2 : 16 * 1.2,
        padding: 20,
        maxLines: 30,
    };
    ...
    function wrapTextAndDraw(context, textToWrap, x, y, maxWidthPx, lineHeightPx, maxLinesAllowed) {
        const words = textToWrap.replace(/\r\n|\r|\n/g, " \n ").split(' '); // Preserve newlines
        let currentLine = '';
        const linesArray = [];
        ...
        for (let n = 0; n < words.length; n++) {
            if (words[n] === '\n') {
                ...
                currentLine = '';
                if (linesArray.length >= maxLinesAllowed) break;
                continue;
            }
            const testLine = currentLine + words[n] + ' ';
            const metrics = context.measureText(testLine.trim()); // Measure trimmed line
            if (metrics.width > maxWidthPx && n > 0 && currentLine.length > 0) {
                linesArray.push(currentLine.trim());
                currentLine = words[n] + ' ';
                if (linesArray.length >= maxLinesAllowed) break;
            } else {
        ...
        if (currentLine.trim()) linesArray.push(currentLine.trim());

        if (linesArray.length > maxLinesAllowed) {
            linesArray.splice(maxLinesAllowed -1, linesArray.length - maxLinesAllowed + 1, linesArray[maxLinesAllowed -1] + "...");
        }
        
        let actualTextWidth = 0;
        linesArray.forEach(line => actualTextWidth = Math.max(actualTextWidth, context.measureText(line).width));
        
        canvas.width = Math.min(maxWidthPx, actualTextWidth) + 2 * config.padding;
        canvas.height = (linesArray.length * lineHeightPx) + 2 * config.padding;

        context.font = config.font; // Re-apply font after canvas resize
        context.fillStyle = config.backgroundColor;
        context.fillRect(0, 0, canvas.width, canvas.height);
        context.fillStyle = config.fontColor;
        context.textAlign = 'left';
        context.textBaseline = 'top';
        linesArray.forEach((line, i) => context.fillText(line, x, y + (i * lineHeightPx)));
        
        return canvas.toDataURL('image/png');
    }

    try {
        return wrapTextAndDraw(ctx, text, config.padding, config.padding, config.maxWidth, config.lineHeight, config.maxLines);
    } catch (e) {
        console.error("Error generating text image:", e);
        // Fallback: create a simple error image
        ctx.canvas.width = 200; ctx.canvas.height = 50;
        ctx.fillStyle = 'white'; ctx.fillRect(0, 0, 200, 50);
        ctx.fillStyle = 'red'; ctx.font = '12px Arial';
        ctx.fillText('Error generating summary', 10, 20);
